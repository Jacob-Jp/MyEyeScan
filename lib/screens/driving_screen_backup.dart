...
class _DrivingScreenState extends State<DrivingScreen>
  ...
  Color get statusColor {
    if (currentDrowsinessLevel >= 0.75) return Colors.red.shade900;
    if (currentDrowsinessLevel >= 0.40) return Colors.orange.shade700;
  ...
  Future<void> _sendSmsAlert(String location) async {
    ...
    // PASO 3: Enviar SMS automáticamente
    await _sendAutomaticSms(contact, messageBody);
  ...
  Future<void> _sendAutomaticSms(String contact, String message) async {
    try {
      final String cleanContact = contact
          .replaceAll(' ', '')
          .replaceAll('+', '');
      print("💬 Enviando SMS automático a: $cleanContact");

      final Uri smsUri = Uri.parse(
        'sms:$cleanContact?body=${Uri.encodeComponent(message)}',
      );

      if (await canLaunchUrl(smsUri)) {
        await launchUrl(smsUri, mode: LaunchMode.externalApplication);
        print("✅ SMS automático enviado");
      } else {
        print("❌ No se pudo enviar SMS automático");
      }
    } catch (e) {
    ...
    showDialog(
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          ...
          actions: [
            // Solo el botón de SMS (sin llamada duplicada)
            SizedBox(
              width: double.infinity,
              child: ElevatedButton.icon(
                onPressed: () => _openSmsAppImproved(contact, message),
  ...
  void _triggerEmergencyAlert(bool activate) async {
    ...
    if (activate) {
      ...
      if (mounted) {
        ...
        // Inicia la vibración de alerta
        _startAlert();

        // Envío de SMS y llamada de emergencia
        _sendSmsAlert(_currentLocation);

        // Después de 3 segundos, ofrecer hacer una llamada
        Future.delayed(const Duration(seconds: 3), () {
          if (_isAlertActive && mounted) {
            _offerEmergencyCall();
          }
        });
      }
  ...
  void _simulateDrowsinessAlert() {
    // Simula que el cansancio se detecta 10 segundos después de iniciar
    Future.delayed(const Duration(seconds: 10), () {
      if (mounted && isServiceRunning /*&& isBluetoothConnected*/ ) {
        _triggerEmergencyAlert(true);

        Future.delayed(const Duration(seconds: 5), () {
          if (mounted) _triggerEmergencyAlert(false);
        });
      }
    });
  }
  ...
  Widget build(BuildContext context) {
    // Configurar el fondo según el estado - MÁS OSCURO
    final backgroundColor = currentDrowsinessLevel >= 0.75
        ? const Color.fromARGB(255, 80, 0, 0) // Rojo muy oscuro para emergencia
        : const Color.fromARGB(255, 8, 8, 20); // Azul muy oscuro / casi negro
                      ...
                      return Container(
                        ...
                        child: ListTile(
                          ...
                          trailing: Row(
                            ...
                            children: [
                              IconButton(
                                onPressed: () => _callEmergencyContact(_emergencyContacts[index]),
                                icon: const Icon(Icons.call, color: Colors.green),
                                tooltip: 'Llamar',
                              ),
                              IconButton(
                                onPressed: () => _removeEmergencyContact(index, setModalState),
  ...
  void _showAddContactDialog(StateSetter setModalState) {
    final TextEditingController nameController = TextEditingController();
    final TextEditingController phoneController = TextEditingController();
      ...
      builder: (context) => AlertDialog(
        ...
        title: const Row(
          children: [
            ...
            SizedBox(width: 10),
            Text('Nuevo Contacto', style: TextStyle(color: Colors.white)),
          ],
        ),
        content: Column(
          mainAxisSize: MainAxisSize.min,
          ...
          ElevatedButton(
            onPressed: () async {
              if (nameController.text.isNotEmpty && phoneController.text.isNotEmpty) {
                await _addEmergencyContact('${nameController.text}|${phoneController.text}');
                setModalState(() {});
                Navigator.pop(context);
              }
            },
  ...
  Future<void> _addEmergencyContact(String contactString) async {
    try {
      final prefs = await SharedPreferences.getInstance();
      final contacts = prefs.getStringList('emergency_contacts') ?? [];
  ...
  Future<void> _removeEmergencyContact(int index, StateSetter setModalState) async {
    try {
      final prefs = await SharedPreferences.getInstance();
      final contacts = prefs.getStringList('emergency_contacts') ?? [];
      if (index < contacts.length) {
